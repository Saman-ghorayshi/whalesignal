...
test("extractCandidatesBTC takes the LARGEST single output, skipping change outputs", () => {
  ...
  const block = {
    ...
    tx: [
      ...
      {
        // two 500-BTC legs to DIFFERENT destinations: each leg is its own
        // $30M whale move — summing to $60M attributed to one address was
        // the old (wrong) behavior that flooded the feed with fake giants
        hash: "bbb",
      ...
      {
        // largest output is change back to a sender address → skipped, the
        // real destination is the second-largest output
        hash: "ccc",
        inputs: [{ prev_out: { addr: "sender4" } }],
        out: [
          { addr: "sender4",     value: 90_000_000_000 }, // change, 900 BTC
          { addr: "true_dest",   value: 10_000_000_000 }, // 100 BTC -> $6M
        ],
      },
      {
        // ALL outputs are change (self-consolidation) → no destination,
        // skip the tx entirely
        hash: "ddd",
        inputs: [{ prev_out: { addr: "sender5" } }],
        out: [{ addr: "sender5", value: 80_000_000_000 }],
      },
    ],
  };
  const out = extractCandidatesBTC(block, MARKET);
  assert.equal(out.length, 3); // ddd has no non-change destination
  ...
  // second: largest single output = 500 BTC × $60K = $30M (not the $60M sum)
  assert.equal(out[1].tx_hash, "bbb");
  assert.equal(out[1].to_address, "intermediate");
  assert.equal(out[1].amount, 500);
  assert.equal(out[1].usd_value, 30_000_000);

  // third: 900-BTC change output skipped → true destination is 100 BTC
  assert.equal(out[2].tx_hash, "ccc");
  assert.equal(out[2].to_address, "true_dest");
  assert.equal(out[2].amount, 100);
  assert.equal(out[2].usd_value, 6_000_000);
});
