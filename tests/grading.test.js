...
import { makeWorld, fullPipeline } from "./e2e.fixture.js";
import { gradePending, gradeWindowCheck, GRADE_EXPIRE_SQL, GRADE_PENDING_SQL } from "../src/bot.js";

const H = 3_600_000;

function seedWhale(DB, { detectedAt, priceAtDetect = 100_000, symbol = "BTC", chain = "btc", from = "0xwhaleFrom0000" }) {
  const r = DB.prepare(
...
function seedAnalysis(DB, whaleId, { signal, confidence = 0.8, createdAt, outcome = null }) {
  DB.prepare(
...
test("grading queries never touch the whales table for planning (read-cap metronome regression)", () => {
  const { env } = makeWorld();
  // enough rows that the planner must reason about real costs, not an empty table
  const now = Date.now();
  for (let i = 0; i < 500; i++) {
    const wid = seedWhale(env.DB, { detectedAt: now - i * H });
    seedAnalysis(env.DB, wid, { signal: i % 2 ? "bullish" : "neutral", createdAt: now - i * H, outcome: i % 2 ? "correct" : "no_signal" });
  }

  for (const [name, sql] of [["expire", GRADE_EXPIRE_SQL], ["pending", GRADE_PENDING_SQL]]) {
    const plan = env.DB.prepare("EXPLAIN QUERY PLAN " + sql).all().results;
  ...
  const inPlan = env.DB.prepare("EXPLAIN QUERY PLAN SELECT id FROM whales WHERE id IN (?,?,?)").all().results;
...
test("gradePending: due call grades against market price and writes rollups", async () => {
  ...
  const wid = seedWhale(env.DB, { detectedAt: now - 25 * H });
  seedAnalysis(env.DB, wid, { signal: "bullish", createdAt: now - 25 * H });
...
test("gradePending: directional move grades correct and counts toward wallet accuracy", async () => {
  ...
  const wid = seedWhale(env.DB, { detectedAt: now - 25 * H, priceAtDetect: 100_000 });
  seedAnalysis(env.DB, wid, { signal: "bullish", createdAt: now - 25 * H });
...
test("gradePending: young calls wait for their 24h mark", async () => {
  const { env } = makeWorld();
  const now = Date.now();
  const wid = seedWhale(env.DB, { detectedAt: now - 2 * H });
  seedAnalysis(env.DB, wid, { signal: "bearish", createdAt: now - 2 * H });
...
test("gradePending: calls past the honest window expire via created_at, never graded late", async () => {
  const { env } = makeWorld();
  const now = Date.now();
  const wid = seedWhale(env.DB, { detectedAt: now - 40 * H });
  seedAnalysis(env.DB, wid, { signal: "bullish", createdAt: now - 40 * H });
...
test("gradePending: late reanalysis of an old whale still expires (detected_at window held in JS)", async () => {
  ...
  const wid = seedWhale(env.DB, { detectedAt: now - 40 * H });
  seedAnalysis(env.DB, wid, { signal: "bullish", createdAt: now - 1 * H });
...
test("gradePending: neutral rows never enter the ungraded set (legacy NULL neutrals stay untouched)", async () => {
  ...
  seedAnalysis(env.DB, wid, { signal: "neutral", createdAt: now - 40 * H, outcome: null });
