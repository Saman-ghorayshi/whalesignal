...
-- Sprint 4: per-wallet time lookups (scanner spam-penalty, analyst wallet
-- history, countCluster, wallet profiles). Without these, every
-- "WHERE from_address = ? AND chain = ?" is a near-full-table scan — the
...
-- ─────────────────────────────────────────────────────────────────────
-- Sprint 4 — stats_cache: one-row payload cache for /stats, /netflow and
-- /graph. Each of those endpoints full-scans the whales table; without
-- this cache a single dashboard visitor polling /stats every 60s burns
-- the D1 free-tier daily read cap (5M rows) in under a day, taking every
-- D1 read on the account down until midnight UTC with it.
-- ─────────────────────────────────────────────────────────────────────
