...
| **Grading metronome fix (Sep 19)** | Per-minute analytics showed 105,201 reads every 15-min grade tick (~420K/h ≈ 10M/day). Root cause: the grading SELECT (`JOIN … ORDER BY w.detected_at`) and the expire UPDATE (`whale_id IN (SELECT id FROM whales WHERE detected_at < ?)`) both let SQLite drive from the 104K-row whales table. Now: expire filters on `analysis.created_at` (no whales join), pending select reads the analysis outcome index first then fetches whale rows by PK, honest 24–36h window enforced in JS via `gradeWindowCheck` (late reanalysis of old whales expires instead of grading against a stale price), vol-threshold price reads only when something is pending | Per-tick reads: ~105K → **<20**. Query-plan regression test in `tests/grading.test.js` (asserts the plans never touch whales) |
| **`/wallet` array-bind fix (Sep 19)** | `walletProfile` bound an ARRAY as one value (`.bind(chain ? [a,b,c] : [a,b])`) — real D1 rejects arrays (`D1_TYPE_ERROR`), so the wallet route 500'd. Now variadic spread | Regression test covers chain and no-chain variants |
| **Scoreboard actually works now (Sep 19)** | `topCalls` selected `a.price_at_detect` (column is on `whales`) → the 18:00 UTC post silently failed every day. Also: `graded24` summed expired/no_data so the posted breakdown didn't add up; `no_data` grades punished wallet track records | All fixed: scoreboard test drives the real 18:00 cron; only correct/wrong/no_move count as grades; unpriceable symbols close as `no_data` without touching counters or `wallet_stats` |
...
**REQUIRED cryptopay-side config**: add to YOUR cryptopay source repo's PACKAGES constant:

```js
"whalesignal_premium": { "price_usd": 10, "days": 30, "gems": 0 },
```

Then redeploy from your source. Do NOT deploy the bundled worker_fixed.js — it's minified and won't parse as source. Reference only.
...
| **Public landing page (Sep 19)** | `GET /` serves a server-rendered conversion page: live accuracy, per-bucket calibration table, recent graded calls, channel CTA, raw-API links, and a "not financial advice" disclaimer. KV-cached 5 min (`landing_html`) so traffic costs ~0 D1 reads; a rebuild is ~6 rollup queries. `/premium` replies now carry the same disclaimer | The "Open the bot" button points at t.me/whalesignal_bot — update if the bot username differs |
