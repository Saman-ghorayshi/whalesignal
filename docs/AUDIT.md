...
| **`/wallet/:addr` was broken by an array bind** (Sep 19, sloppy sweep) | `walletProfile` called `.bind(chain ? [a,b,c] : [a,b])` — ONE array argument for 2-3 placeholders. D1 rejects arrays as bind values (D1_TYPE_ERROR); the "revert" in the previous session re-introduced the same pattern it was fixing | ✅ variadic spread (`.bind(...txBinds)`) + regression test with and without the chain filter |
| **The daily scoreboard has NEVER posted** (Sep 19, sloppy sweep) | Its `topCalls` query selected `a.price_at_detect` — that column lives on `whales`, not `analysis`. `no such column` → swallowed by the cron's try/catch every day at 18:00 UTC | ✅ `w.price_at_detect`; scoreboard test now actually runs the 18:00 cron path through the harness |
| **MockD1 lied about D1's API** (Sep 19, sloppy sweep) | `.first()`/`.all()`/`.run()` returned plain values, not Promises — worker code doing `.first().catch(...)` (valid on real D1) crashed the harness, and `.run()` inside tests worked un-awaited, hiding real async behavior | ✅ all three terminators are async, like real D1; tests awaited everywhere |
| **`no_data` polluted the accountability ledger** (Sep 19, sloppy sweep) | Unpriceable symbols graded as `no_data` still incremented outcome counters and the wallet's graded/correct record — punishing wallets for OUR missing price data | ✅ ledger row still closes (honest), but counters + wallet_stats skip `no_data`; `postScoreboard` counts only correct+wrong+no_move, so its own breakdown finally adds up |
